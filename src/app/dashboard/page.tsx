    ...
    <main style={{ maxWidth: 480, margin: '0 auto', minHeight: '100svh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      <div style={{ padding: '52px 20px 28px', textAlign: 'center', borderBottom: '1px solid var(--border)', background: 'linear-gradient(180deg,rgba(200,149,58,.05),transparent)' }}>
        ...
        <div style={{ marginTop: 7, fontSize: 13, color: 'var(--dim)', fontStyle: 'italic' }}>
          {STEMS[dmSi]} {STEM_META[dmSi].element} Day Master · Kua {user.kua_number || 1}
        </div>
        <button
          onClick={() => { localStorage.removeItem('bazi_user'); router.push('/') }}
          style={{ position: 'absolute', top: 16, right: 16, background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 20, padding: '6px 14px', fontSize: 11, color: 'var(--dim)', cursor: 'pointer', fontFamily: 'Cinzel,serif', letterSpacing: '.1em' }}
        >Keluar</button>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 20px' }}>
        ...
        <button
          ...
          style={{ width: '100%', maxWidth: 300, background: 'var(--surface)', border: '1px solid var(--gl)', borderRadius: 22, cursor: 'pointer', overflow: 'hidden', transition: 'all .3s', fontFamily: 'inherit' }}
        >
      ...
      {showReading && (
        <ReadingSheet user={user} si={today.si} bi={today.bi} onClose={() => setShowReading(false)} />
      )}
    </main>
  )
}
...
function ReadingSheet({ user, si, bi, onClose }: { user: UserProfile; si: number; bi: number; onClose: () => void }) {
  ...
  const ec = (el: string) => EL_COLORS[el] || '#c89040'

  const stemEl   = sm.element
  const branchEl = bm.element
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--bg)', overflowY: 'auto', zIndex: 100, animation: 'fadeUp .4s ease forwards' }}>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 48 }}>
        ...
        {/* Pillar Hero */}
        <div style={{ background: 'var(--surface)', border: '1px solid var(--b2)', borderRadius: 22, padding: '26px 16px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-end', gap: 18, marginBottom: 10 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 66, lineHeight: 1, background: 'linear-gradient(180deg,#e8c060,#c8953a,#5a3808)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{STEMS[si]}</div>
              <div style={{ fontSize: 11, color: 'var(--dim)', marginTop: 4 }}>{sm.romanization} · {sm.element}</div>
            </div>
            <div style={{ fontSize: 36, color: 'var(--b2)', paddingBottom: 14 }}>·</div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 66, lineHeight: 1, background: 'linear-gradient(180deg,#e8c060,#c8953a,#5a3808)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{BRANCHES[bi]}</div>
              <div style={{ fontSize: 11, color: 'var(--dim)', marginTop: 4 }}>{bm.romanization} · {bm.animal}</div>
            </div>
          </div>

          {/* Element pills — no more array access, using named properties */}
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 8 }}>
            <span style={{ padding: '4px 16px', borderRadius: 20, fontSize: 12, color: ec(stemEl), border: `1px solid ${ec(stemEl)}40`, background: `${ec(stemEl)}18` }}>{stemEl}</span>
            <span style={{ padding: '4px 16px', borderRadius: 20, fontSize: 12, color: ec(branchEl), border: `1px solid ${ec(branchEl)}40`, background: `${ec(branchEl)}18` }}>{branchEl}</span>
          </div>
          ...
          {reading && (
            <>
              ...
              <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 14px', textAlign: 'left', marginBottom: 10 }}>
                ...
                <p style={{ fontSize: 14, lineHeight: 1.72, color: 'var(--dim)', marginBottom: 6 }}>
                  <strong style={{ color: 'var(--teal)' }}>Harmoni:</strong> {reading.harmony}
                </p>
                <p style={{ fontSize: 14, lineHeight: 1.72, color: 'var(--dim)' }}>
                  <strong style={{ color: 'var(--red)' }}>Konflik:</strong> {reading.conflict}
                </p>
              </div>
            </>
          )}
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 15, overflow: 'hidden' }}>
      ...
      <div style={{ padding: 16 }}>
        <p style={{ fontSize: 15, lineHeight: 1.8, color: 'var(--text)' }}>{data.intro}</p>
        <button
        ...
        >
          ...
          <span style={{ fontSize: 11, color: open ? 'transparent' : 'var(--dim)', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: '2px 10px' }}>{data.cells.length} poin</span>
...
function DetailCell({ cell }: { cell: Cell }) {
  ...
  return (
    <div style={{
      ...
      borderTop: `2px solid ${col}`, borderRadius: 12, padding: '12px 11px',
      ...
      {cell.tag && (
        <span style={{
          display: 'inline-block', padding: '2px 9px', borderRadius: 10, fontSize: 11, marginTop: 6,
...
function ScoreCard({ score }: { score: number }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--b2)', borderRadius: 18, padding: 18, display: 'flex', alignItems: 'center', gap: 16 }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: '.2em', color: 'var(--dim)', textTransform: 'uppercase', marginBottom: 7 }}>Skor Keberuntungan Hari Ini</div>
      ...
      {open && (
        <div style={{ padding: '0 16px 16px' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--dim)', lineHeight: 1.65, fontStyle: 'italic' }}>
            {STEMS[getDayMasterSi(user.birth_date)]} {STEM_META[getDayMasterSi(user.birth_date)].element} Day Master · {user.birth_date} · Kua {user.kua_number || '—'}<br />
